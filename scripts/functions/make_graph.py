...
def plot_angular_velocity(angle_list, angular_velocity_list, day):
    ...
    for i in range(sample_num):
        ...
        axs[row, col].set_xlabel("time [s]")
        axs[row, col].set_ylabel("anglular velocity [rad/s]")
    plt.tight_layout()
    ...
    for i in range(sample_num):
        ...
        if flag_add_peak:
            ...
            axs[row, col].text(freq_at_max_amp + 15, max(Amp_list[i]) * 0.5, f"Peak: {freq_at_max_amp:.2f} Hz", 
                           color="r", verticalalignment="bottom", horizontalalignment="right")
        axs[row, col].grid(True)
...
def plot_SD_list(SD_list, day, flag_std):
    ...
    for i, width_time in enumerate(width_time_list):
        ...
        for j in range(sample_num):
            ...
            axs[row, col].grid(True)
            if flag_std:
                axs[row, col].set_title(f"Standardized SD Time-series No.{j+1}")
                axs[row, col].set_ylabel("Standardized SD")
            else:
...
def plot_validation1(angle_list, angular_velocity_list, day):
    ...
    for i in range(sample_num):
        ...
        axs[row, col].set_title(f"Anglar Velocity time-series No.{i+1}")
        axs[row, col].set_xlabel("time [s]")
        axs[row, col].set_ylabel("angle / anglular velocity")
    plt.tight_layout()
    plt.savefig(f"{save_dir}/angle__anglular_velocity.png")
    plt.close(fig)


# dev validation2: 回転中心と重心座標の距離の平均 vs 角速度のSD
def plot_validation2(x_list, y_list, angular_velocity_list, day):
    ...
    plt.xlabel("Average Distance from Center")
    plt.ylabel("Standard Deviation of Angular Velocity")
    plt.title("Average Distance from Center vs. Angular Velocity SD")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
